...
const Dashboard = () => {
  ...
  const menuItems = [
    ...
    { name: 'Finance & Report', path: '/dashboard/finance', icon: BarChart3 },
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="hidden md:flex bg-white px-6 py-3 items-center justify-end">
          <div className="relative" ref={userMenuRef}>
            ...
            {/* Dropdown Menu */}
            {isUserMenuOpen && (
              <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50 animate-in fade-in zoom-in duration-200">
                <div className="px-4 py-3 border-b border-gray-100">
                  <p className="text-sm font-bold text-gray-900 truncate">{user?.username || 'User'}</p>
                  <p className="text-xs text-gray-500 truncate">{user?.email || 'user@example.com'}</p>
                  <p className="text-[10px] uppercase tracking-wider font-semibold text-primary mt-1">{user?.role || 'Admin'}</p>
                </div>
                <button 
                  onClick={handleLogout}
        ...
        <main className="flex-1 overflow-auto p-6 bg-white">
          <Routes>
            <Route path="/" element={<PlaceholderPage title={`Dashboard ${user?.role || 'Admin'}`} />} />
            <Route path="/inventory" element={<PlaceholderPage title={`Inventory ${user?.role || 'Admin'}`} />} />
            <Route path="/finance" element={<PlaceholderPage title={`Finance & Report ${user?.role || 'Admin'}`} />} />
            <Route path="/settings" element={<PlaceholderPage title={`Settings ${user?.role || 'Admin'}`} />} />
