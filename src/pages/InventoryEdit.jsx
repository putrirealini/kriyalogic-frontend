...
import useUpdateParentProduct from '../hooks/useUpdateParentProduct';
...
const InventoryEdit = () => {
    ...
    const { parentProduct, loading: detailLoading, error: detailError } = useDetailParentProduct(id);
    const { updateParentProduct, loading, error, setError } = useUpdateParentProduct();
    ...
    useEffect(() => {
        ...
        if (parentProduct.logo) {
            setLogo({
                id: `existing-logo-${parentProduct._id || parentProduct.id || 'parent'}`,
                file: null,
                previewUrl: parentProduct.logo,
                imageUrl: parentProduct.logo,
                isExisting: true
            });
        } else {
            setLogo(null);
    ...
    useEffect(() => {
        return () => {
            if (logo?.previewUrl && !logo?.isExisting) {
    ...
    const buildPayload = async () => {
        ...
        if (logo) {
            if (logo.isExisting) {
                logoValue = logo.imageUrl || logo.previewUrl || '';
            } else if (logo.file) {
                logoValue = await fileToBase64(logo.file);
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await updateParentProduct(id, payload);
            ...
            if (result.success) {
                ...
                toast.error(result.error || 'Failed to update parent product');
            }
        } catch (err) {
    ...
    if (detailError) {
        return <div className="p-6 text-center text-red-600">Error: {detailError}</div>;
    }
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="relative" ref={categoryDropdownRef}>
                                    ...
                                    {categoriesError && (
                                        <div className="mt-2 rounded-xl border border-red-200 bg-white shadow-md bg-red-50 px-3 py-2 text-sm text-red-600">
                                            {categoriesError}
                                        ...
                                        <div className="absolute z-20 mt-2 w-full rounded-2xl bg-white shadow-xl border border-gray-100 overflow-hidden">
                                            <div className="p-3 border-b border-gray-100">
                                                <input
                                                    ...
                                                    className="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none focus:ring-2 focus:ring-[#6A4734]"
                                                    autoFocus
                                                />
